    ...
    const refresh = () => {
      return new Promise((resolve) => {
        ...
        if (route.value.params.id) {
          socket.emit('generic::getOne', { id: route.value.params.id, parseText: true }, (err: string | null, cb: { external: string, text: string, js: string, css: string, parsedText: string }) => {
            if (err) {
              return console.error(err);
            }
            if (!cb) {
              return console.warn('No text overlay found with id ' + route.value.params.id);
            }
            if (!external.value) {
              if (cb.external) {
